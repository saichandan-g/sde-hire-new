...
export function HRReportsTab() {
  ...
  const avgOverallScore = hrReports.length > 0
    ? Math.round(hrReports.reduce((sum, report) => {
        const score = report.hr_evaluation?.overallScore || 0
        return sum + score
      }, 0) / hrReports.length)
    : 0

  const highestScore = hrReports.length > 0
    ? Math.max(...hrReports.map(report => report.hr_evaluation?.overallScore || 0))
    : 0

  const recentScore = hrReports.length > 0
    ? hrReports[0]?.hr_evaluation?.overallScore || 0
    : null
              ...
              <TableBody>
                {hrReports.map((report) => (
                  <TableRow key={report.id}>
                    ...
                    <TableCell>
                      <Badge variant={getScoreVariant(report.hr_evaluation?.overallScore || 0)}>
                        {report.hr_evaluation?.overallScore || 0}%
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline">
                        {report.hr_evaluation?.confidenceLevel || 0}%
                      </Badge>
                    </TableCell>
                    <TableCell>
